...
- **User Scoping**: All data is scoped to the authenticated user. RLS policies ensure users can only access and manage their own data. `auth.uid()` is used in RLS policies to refer to the ID of the currently authenticated user.
